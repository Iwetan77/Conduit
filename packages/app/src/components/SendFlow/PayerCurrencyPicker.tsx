...
export function PayerCurrencyPicker({ value, onChange }: PayerCurrencyPickerProps) {
  ...
  if (held.length === 0) {
    return (
      <div className="flex flex-col gap-1.5">
        <label className="text-xs font-mono text-ink-dim uppercase tracking-wider">You pay with</label>
        <p className="text-danger text-sm">No routable balance found in this wallet on Arc Testnet.</p>
      </div>
    );
  }
